...
List<MuseumPoint> _getMapPointsM() {
  return _readMuseumPointsFromFile('assets/museums.txt');
}

List<MuseumPoint> _readMuseumPointsFromFile(String filePath) {
  List<MuseumPoint> points = [];

  try {
    String fileContents = File(filePath).readAsStringSync();
    List<String> lines = LineSplitter.split(fileContents).toList();
    ...
    for (String line in lines) {
      ...
      if (values.length == 3) {
        ...
        double latitude = double.parse(values[1].trim());
        double longitude = double.parse(values[2].trim());

        points.add(MuseumPoint(name: name, latitude: latitude, longitude: longitude));
      }
    }
  } catch (e) {
